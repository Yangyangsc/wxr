var noticeInterval = 3, //答题须知界面显示停留时间
  ...
  wholeOnMusic = document.getElementById('wholeOnMusic'), //开门321关门
...
if (genderStr && postStr) {
  ...
  //动态获取职业类型
  $.get(http + '/getinitdata', {
    uid: userid
  }, function (result) {
    if (result.data.info_is_complete) {
      //用户已注册，跳过选择资料界面，进入答题
      $('.perfect').hide();
      $('.homeBox').css('display', 'flex');
      localStorage.setItem('gender', result.data.user_sex); //获取性别对应值
      genderStr = localStorage.getItem('gender');
      localStorage.setItem('post', result.data.user_job); //获取职业对应值
      postStr = localStorage.getItem('post');
      if (result.data.user_sex == 1) {
        $('.peopleManBox').show();
        $('.peopleGirlBox').hide();
      } else if (result.data.user_sex == 0) {
        $('.peopleGirlBox').show();
        $('.peopleManBox').hide();
      }
    } else {
      //用户未注册，进入选择资料界面，渲染职业
      for (const index in result.data.jobs) {
        $("#selectBox_work").append(`<div class="post postOff" data-type="${index}">${result.data.jobs[index].job_name}</div>`);
      }
      //选择职业方法因👆方法而需要改用👇方法
...
//动态获取游戏规则 答题须知
$.get(http+'/getgamerule',{},(res)=>{
  $("#game_rule").html(res.data[0].rule);
})
...
m_testBut.click(function () {
  ...
  if (!music.paused) {
    music.pause();
    audioAutoPlay("wholeOnMusic")
  }
  //   setTimeout(function() {
  //     countDownMusic.play()
  //     audioAutoPlay("countDownMusic")
  //   }, 4000)
  setTimeout(function () {
    // audioAutoPlay("openDoor")
    starting();
  }, 5000);
  answerShowFun();
});
...
var flag = true;
//选择题目
$('.answerBoxs').on('click', '.topicBox ul li', function () {
  if (flag) {
    flag = false;
    $('#leftDoor').removeClass('leftDoorOff leftDoorOff1');
    $('#rightDoor').removeClass('rightDoorOff rightDoorOff1');
    var that = $(this);
    var topParents = that.parents('.topicBox'); //获取题目父级元素
    var topParentsEq = topParents.index(); //获取当前题目位置
    var toParentOne = topParentsEq + 1; // 题目从1开始
    var location = toChinesNum(toParentOne + 1); //题目位置中文
    if (that.attr("data-ans") == answerData[topParentsEq].ans) {
      that.prop('class', 'answerOn');
    } else {
      that.prop('class', 'answer_wrong');
    }
    if (!isSubmit) {
      ...
      var answerOnTime = setTimeout(function () {
        ...
        $('.answerInfo .omit').eq(1).text(answerType[answerData[topParentsEq].cat_id - 1] + '篇');
      ...
      resultData.data.push({
        qid: answerData[topParentsEq].id,
        answer: letter[$(this).index()]
      })
      if (toParentOne == answerLen) {
        portion();
      }
    }
  }
...
function createPoster(count, defeat) {
  ...
  setTimeout(function () {
    ...
    canvas_text(ctx, parseInt(defeat.substring(0, defeat.length - 1)) + '%', times(percentage_fontSize).toString() +
      'px bold 黑体', percentage_fontColor, times(112), shrink(1.66), 1);
  ...
  $.get(http + '/starting', {
    ...
    for (var i = 0; i < answerData.length; i++) {
      ...
      var str =
        ...
        '</div>' +
        '<img  onclick="handleEject(this)" id="Big_img' + i + '" src="' + data.img + '" alt="img">' +
        '<ul>' +
        '<li data-ans="A" class="answerOff"><span>' + data.opt1 + '</span></li>' +
        '<li data-ans="B" class="answerOff"><span>' + data.opt2 + '</span></li>' +
        '<li data-ans="C" class="answerOff"><span>' + data.opt3 + '</span></li>' +
        '<li data-ans="D" class="answerOff"><span>' + data.opt4 + '</span></li>' +
        '</ul>' +
        '</div>'
      $('.answerBoxs').append(str);
      var src = $(`#Big_img${i}`).attr("src")
      if (!src) {
        $(`#Big_img${i}`).css("display", "none");
        $(`#Big_img${i}`).parent().children()[0].style.height = "1.7rem"
      } else {
        $(`#Big_img${i}`).parent().children()[0].style.height = ""
      }
    }
...
function handleEject(event) {
  window.event ? window.event.cancelBubble = true : e.stopPropagation();
  var display = $(".img-swiper").css("display")
  var src = event.getAttribute("src")
  if (display == "none") {
    $(".img-swiper").fadeIn(500)
    $(".img-swiper-eject img").attr("src", src);
  } else {
    $(".img-swiper").fadeOut(500)
  }
}
...
function checkanswer() {
  $.post(http + '/checkanswer', {
    ...
    isSubmit = false;
    $('.schedule').hide();
    $('#textScore').text(parseInt(result.defeat.substring(0, result.defeat.length - 1)) + '%')
    $('#user_number').text(resultData.result_id)
    $('#pmsNum').text(result.allrightph)
    if (result.allrightph != 0) {
    ...
    if (result.get_yhq == 0) {
      $(".buyBut").hide();
      $(".butContainer").css("justify-content", "center");
    }
    answerBox.hide();
    resultBox.show();
