...
　       if (!RegExp.prototype.isPrototypeOf(findWhat)) { 
    ...
    addCache: function (key, value, options) {
        ...
        var opt = $.extend(true, { expires: 7, path: '/' }, options)
    ...
    getActivityShareUrl:function(activityid,channelid,giftid){
        var baseUrl = 'http://activity.litecoder.com/?';
    ...
    ///初始化微信分享
    createWXShareInit:function(shareOption){
        if (shareOption==null){
            shareOption =JSON.parse(common.getCache('shareOption'));
        }
        wechatHelper.init(function () {
            wechatHelper.share(shareOption.title || '弯弓活动', 
                               shareOption.content || '弯弓活动', 
                               
                               common.getActivityShareUrl(shareOption.activityid  , shareOption.channelid,shareOption.giftid),
                               shareOption.image)
      });
    }
}