			...
			$.each(this.options.data, function(i, item) {
				...
				html.push(
					'<a class="parent" href="' + (item.url || 'javascript:void(0)') + '">',
						'<img style="padding:2px;border:0px" width="32" height="32" src="' +urlConfig.base.imageBase+ item.icon + '">',
						'<span>' + item.name + '</span>',
					'</a>'
				);
				if (item.children && item.children.length) {
					html.push('<ul>');
					//data-name="{title:\'' + item.name + '\',url:\'' + item.url + '\',target:\''+item.target+'\'}"
					html.push(
						'<li>',
					...
					$.each(item.children || [], function(i, c) {
						html.push(
							'<li>',
                                ...
                                '<a  data-name="{title:\'' + c.name + '\',url:\'' + c.url + '\',target:\''+c.target+'\'}" href="#">',
									c.name,
								'</a>',
							'</li>'
						);
