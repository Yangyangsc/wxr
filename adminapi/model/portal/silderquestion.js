...
class SliderModel extends daoBase {
    get getByIdSql() {return "select * from  crm_user_yhq_qa where slider_id=? ";}
    get updateSql() {return "update crm_user_yhq_qa set ? where slider_id=?";}
    get insertSql() {return "insert into crm_user_yhq_qa set ?";}
    get deleteSql() {return "update crm_user_yhq_qa set rec_isdeleted=1 WHERE find_in_set(slider_id,?)";}
    ///根据用户登录账号获取用户
    get getByKey() {
        return "select count(1) as total from portal_slider where slider_key=? and rec_isdeleted=0"
    }
    ///根据主键获取一条记录
    getBykey(Sql,id) {
