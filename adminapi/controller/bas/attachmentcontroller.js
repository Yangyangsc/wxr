...
class AttachmentController extends controllerBase {
    ...
    */
    constructor(application) {
        ///将对应的数据库实体对象传递至基类
        super(application, new attachmentDao());
    }
    /*
    * 初始化控制器中的路由
    ...
    */
    initializeRouter(instance) {
        ...
        ///根据实际情况可以继续添加其他路由，或删除现有路由
        //#处理获取实体列表的路由函数
        this.app.get('/attachment', function (req, res) {
            co(instance._daoModel.getAttachmentsByBusiness(req.query.busType,req.query.busId))
                .then(result =>res.json(result))
                .catch(result => { console.log(result); instance.responseResult(req,res,result); })
        });
        //#处理提交新增实体记录的路由配置
        this.app.post('/attachment', function (req, res) {
            co(instance.create(req, "attachment"))
                .then(result => { instance.responseResult(req,res,result) })
                .catch(result =>{ instance.responseResult(req,res,result) });
        });
        //#处理删除实体记录的路由配置
        this.app.delete('/attachment/:id', function (req, res) {
...
exports = module.exports = AttachmentController;