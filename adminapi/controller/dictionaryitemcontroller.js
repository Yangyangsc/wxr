...
*/
var co = require('co');
var dictionaryitemDao = require('../model/dictionaryitem');
...
class DictionaryitemController extends controllerBase {
    ...
    */
    initializeRouter(instance) {
        ...
        ///根据实际情况可以继续添加其他路由，或删除现有路由
        //#处理获取实体列表的路由函数
        this.app.get('/dictionaryitem', function (req, res) {
        ...
        //#处理提交新增实体记录的路由配置
        this.app.post('/dictionaryitem', function (req, res) {
            co(instance.create(req, "dictionaryitem"))
                .then(result => { instance.responseResult(req,res,result) })
                .catch(result =>{ instance.responseResult(req,res,result) });
        });
        //#获取详细记录的路由配置
        this.app.get('/dictionaryitem/:id', function (req, res) {
            co(instance.getDataById(req, "dictionaryitem"))
                .then(result => instance.responseResult(req,res,result))
                .catch(result => {instance.responseResult(req,res,result);});
        });
        //#处理更新实体记录的路由配置
        this.app.put('/dictionaryitem/:id', function (req, res) {
            co(instance.update(req, "dictionaryitem"))
                .then(result => instance.responseResult(req,res,result))
                .catch(result=> { instance.responseResult(req,res,result); });
        });
        //#处理删除实体记录的路由配置
        this.app.delete('/dictionaryitem/:id', function (req, res) {
