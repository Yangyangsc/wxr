...
class controllerBase {
    ...
    *create(req, dataKey,sqlCommand) {
        ...
        let beforeData =yield this.beforeAccessDB(req, sqlCommand || this._daoModel.insertSql, resultItem.data, "create");
        ////如果返回Null,则表示不加载数据了
        if (beforeData.canceled) return {successed:false,errorcode:1,errormessage:"canceled"};
    ...
    *delete(req, id,sqlCommand) {
        ...
        ///从Request中获取到用户提交的数据
        let beforeData =yield this.beforeAccessDB(req, sqlCommand||this._daoModel.deleteSql, id, "delete");
